...
function blockDriverLockedVehicle( thePlayer, seat, jacked )
    ...
    if toggleLock[source] and vehOwner[source] ~= thePlayer and not exports.GTIgroups:arePlayersInSameGroup(thePlayer,vehOwner[source]) and not exports.GTIcontactsApp:arePlayersFriend(thePlayer,vehOwner[source]) then 
        cancelEvent()
