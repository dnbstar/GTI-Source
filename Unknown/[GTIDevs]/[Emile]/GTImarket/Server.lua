...
function setItemForSale(item, units, price, stype)
	...
	if stype == "Ammo" and getWeaponIDFromName(item) == 37 then dm("Due to a MTA bug, you cannot sell flamethrower ammo.",client,255,0,0) return end
...
function buyItem(item, priceunit, amount, seller, stype)
...
function showGui(hit)
    if ( getElementType(hit) == "player" ) and not isPedInVehicle(hit) then
        ...
        triggerClientEvent(hit, "GTImarket.showGui", hit, market, ammo, weps)
    end
end
