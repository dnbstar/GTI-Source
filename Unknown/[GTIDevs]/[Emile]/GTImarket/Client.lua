...
local sale = {}
local blips = {--ped loc xyz,rot
{1089.975, -1511.897, 15.801, 287.712},
{1986.689, 2066.069, 10.831, 352.138},
}
...
addEventHandler ( "onClientGUIClick", root,
    function (_, state )
            if ( source == gTable.button[1] ) then -- 'Buy'
                ...
                if not( row ~= -1 or col ~= 0 ) then dm("Select the item you wish to buy first", 255, 125, 0) return end
                    ...
                    if not( getPlayerName(localPlayer) == seller )then
                        ...
                        if ( amount ~= "" and amount <= units ) then
                            ...
                            triggerServerEvent("GTImarket.buyItem", localPlayer, item, tonumber(price), tonumber(amount), getPlayerFromName(seller), stype)
							guiSetText(gTable.edit['atb'],"")
							buyButton = "Purchase"
        ...
        elseif ( source == gTable.button[2] ) then -- 'Put for Sale'
            ...
            if not ( row ~= -1 ) then return dm("Select the item you wish to put on sale first", 255, 125, 0) end
...
function gotItems(market,ammo,weps)
...
function show(market,ammo,weps)
	showGuis(true)
    buyButton = "Purchase"
	guiSetText(gTable.button[1],buyButton)
    putButton = "Sell"
	guiSetText(gTable.button[2],putButton)
	gotItems(market,ammo,weps)
	status = true
end
...
addEventHandler ( "onClientResourceStart", root,
    function(res)
    if (res == getThisResource()) or (res == getResourceFromName("GTIblips")) then
        ...
        for i,v in ipairs ( blips ) do
			exports.GTIblips:createCustomBlip ( v[1], v[2], 16, 16, "font/market.png", 500 )
			local ped = createPed(28,v[1],v[2],v[3],v[4])
