...
function func_ircKick (user,reason)
	local userName = ircGetUserNick( user)
	if userName then
		if reason then
			return ircRaw(ircGetUserServer( ircGetUserFromNick( "Kayla")), "KICK "..userName.." "..reason)
		else
			return ircRaw(ircGetUserServer( ircGetUserFromNick( "Kayla")), "KICK "..userName)
		end
	end
	return false
end
registerFunction("ircKick","func_ircKick","irc-user","string")
