		...
		if  aclFile then
			...
			while(xmlFindChild(aclFile,"command",i)) do
				...
				if commands[string.lower(attrs.name)] then
					...
					commands[string.lower(attrs.name)].desc = tostring(attrs.desc)
					commands[string.lower(attrs.name)].echoChannelOnly = attrs.echoChannelOnly
				else
					commands[string.lower(attrs.name)] = {level = tonumber(attrs.level),echoChannelOnly = attrs.echoChannelOnly,desc = attrs.desc}
				end
				i = i + 1
			end
