...
addEventHandler("onIRCResourceStart",root,
	function ()
		...
		addIRCCommandHandler("!join",
			function(server,channel,user,command, channelName, password)
				if not channelName then ircNotice(user, "Syntaz is !join <channelName> <password (Optional)>") return end
				if string.find( channelName, "#") then
					channelName = string.gsub( channelName, "#", "")
				end
				if not password then
					ircJoin( server, "#"..channelName)
				else
					ircJoin( server, "#"..channelName, password)
				end
			end
		)

		addIRCCommandHandler("!imute",
			function(server,channel,user,command, victim, reason)
				if not victim then ircNotice(user, "Syntaz is !join <user> <reason>") return end
				if ircGetUserFromNick( victim) then
					local vic = ircGetUserFromNick( victim)
					--ircSetUserMode( vic, "+m")
					if reason then
						--ircKick( vic, reason)
						ircRaw( server, "kick #echo "..victim.." "..reason)
						ircSay( channel, victim.." kicked by Kayla ( "..reason..").")
					else
						--ircKick( vic)
						ircRaw( server, "kick #echo "..victim)
						ircSay( channel, victim.." kicked by Kayla.")
					end
				else
					ircNotice( user, "'"..victim.."' no such user")
				end
			end
		)

		addIRCCommandHandler("!leave",
			function(server,channel,user,command, channelName, reason)
				if not channelName then ircNotice(user, "Syntaz is !leave <channelName> <reason (Optional)>") return end
				if string.find( channelName, "#") then
					channelName = string.gsub( channelName, "#", "")
				end
				if reason then
					ircPart( ircGetChannelFromName( "#"..channelName), reason)
				else
					ircPart( ircGetChannelFromName( "#"..channelName), "Leaving")
				end
			end
		)
				...
				for k,v in ipairs(usage) do
					...
					if (_usage ~= nil) then
						...
						--if (tonumber(_usage) > 50) then
							...
							ircNotice( user, "[Resource Usage] '"..v[1].."' is using "..tostring(_usage).."% CPU usage.")
						--end
					end
				end
			end
		...
		function outputNewCommands (server,channel,user,command)
			if ircIsEchoChannel(channel) then
				ircNotice(user,ircGetUserNick(user)..", you can use these commands: ")
				for i,cmd in ipairs (ircGetCommands()) do
					if ircIsCommandEchoChannelOnly(cmd) then
						if (tonumber(ircGetCommandLevel(cmd) or 6)) <= (tonumber(ircGetUserLevel(user,channel)) or 0) then
							local desc = ircGetCommandDescription(cmd)
							ircNotice(user, "- "..cmd..": "..desc)
						end
					else
						if ircGetCommandLevel(cmd) <= ircGetUserLevel(user,channel) then
							local desc = ircGetCommandDescription(cmd)
		...
		addIRCCommandHandler("!newcmds",outputNewCommands)
...
--]]
		...
		addIRCCommandHandler("!gti",
			function( server, channel, user, command, ...)
				local text = table.concat( {...}, " ")
				if text == "" then ircNotice( user, "syntax is !gti <text>") return end
				if ircGetChannelName( channel) == "#Staff" then
					for index, players in pairs(getElementsByType("player")) do
						if (exports.GTIstaff2:isPlayerStaff(players)) then
							outputChatBox("#FF0000(#Staff) "..ircGetUserNick(user).. ": #FFFFFF"..text, players, 255, 255, 255, true)
						end
					end
				end
			end
