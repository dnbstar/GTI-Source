...
function createEverything()
	for i, npc in ipairs (swat) do
		...
		if isElement( plr) then
			...
			state[ped] = "idle"
			--addEventHandler( "onClientPedDamage", ped, NPCDamage)
			addEventHandler( "onClientPedWasted", ped, NPCDeath)
	...
	if getPedTask( owner, "primary", 3) == "TASK_COMPLEX_ENTER_CAR_AS_DRIVER" then
		if not state[npcs[owner]] or state[npcs[owner]] == "idle" then
			if not isPedInVehicle( npcs[owner]) then
				--outputDebugString( "warp to vehicle")
				warpPedIntoVehicle( npcs[owner], getPedOccupiedVehicle( owner))
...
function followPlayer(player, cop)
	...
	--[[
	if getPedSimplestTask( cop) == "TASK_SIMPLE_JUMP" then
		setPedControlState(player, "jump", true)
		--setPedControlState(player, "jump", false)
		setTimer( setPedControlState, 100, 1, player, "jump", false)
	end
	--]]
	...
	elseif (dist > 8) then
			-- Sprint
		setPedControlState(player, "sprint", true)
		setPedControlState(player, "walk", false)
		setPedControlState(player, "forwards", true)
		setPedControlState(player, "jump", true)
		setTimer(followPlayer, responseTime, 1, player, cop)
	elseif (dist > 4) then
			-- Jog
	...
	elseif (dist <= 1) then
		...
		setPedControlState(player, "sprint", false)
		setPedControlState(player, "walk", false)
		setPedControlState(player, "forwards", false)
		setTimer(followPlayer, responseTime, 1, player, cop)
	end
end
--]]
