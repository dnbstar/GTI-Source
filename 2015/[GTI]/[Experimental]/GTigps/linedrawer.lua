...
function removeLinePoint( posX, posY)
	...
	for name2, data in pairs ( renderStuff) do
		if string.find(name2, name) then
			unloadTile( name2)
			--
			local points = linePoints [ name2]
			points[1].posX, points[1].posY = false, false
		end
	end
end
...
function addLinePoint ( posX, posY )
	...
	-- Now add this point
	table.insert ( linePoints[name], { posX = tileX, posY = tileY, dxID = id} )
...
function loadTile ( name )
	...
	shader = dxCreateShader ( "overlay.fx" )
	if not shader then
		return false
	end
