...
addEventHandler("onClientPreRender", root,
    function()
		...
		dxDrawImage( 65, 150, 312, 79, idir.."logo.png", 0, 0, 0, tocolor(255, 255, 255), false)
		...
		for i, tattoo in ipairs ( vTable) do
			...
			if tattoo[3] and string.find( tattoo[3], ",") then
				local rest2 = tonumber((238.5+(move/2))+(offs*i))
				local color = split( tattoo[3], ",")
				local cR, cG, cB = color[1], color[2], color[3]
				if rest2 >= 263 and rest2 <= 540 then
					if i == selShow then
						dxDrawImage( 72, (238.5+(move/2))+(offs*i), 24, 24, idir.."nut2.png", 0, 0, 0, tocolor( cR, cG, cB, 255), false)
					else
						dxDrawImage( 72, (238.5+(move/2))+(offs*i), 24, 24, idir.."nut.png", 0, 0, 0, tocolor( cR, cG, cB, 255), false)
					end
				end
			end
			if restriction >= 269 and restriction <= 809 then
				...
				if price == "-" then
					...
					dxDrawImage( 342, (234+move)+(offs*i), 32, 32, idir.."tte.png", 0, 0, 0, tocolor( 255, 255, 255, 200), false)
				elseif price and type(items[price]) == "table" then
					...
					if i == selShow then
						dxDrawImage( 342, (234+move)+(offs*i), 32, 32, idir.."t2e.png", 0, 0, 0, tocolor( 255, 255, 255, 190), false)
					else
						dxDrawImage( 342, (234+move)+(offs*i), 32, 32, idir.."tte.png", 0, 0, 0, tocolor( 255, 255, 255, 190), false)
					end
				else
...
function rotateCamera()
	if not viewing[localPlayer] then return end
	if getPlayerName( localPlayer) ~= "LilDolla" then return end
    local x, y, z = getElementPosition(localPlayer)

	local camX = x + math.cos( facing / math.pi * 180 ) * 8.8
	local camY = y + math.sin( facing / math.pi * 180 ) * 8.8 -- 4.8
	setCameraMatrix( camX, camY, z+1, x, y, z )
	facing = facing + 0.00015
end
