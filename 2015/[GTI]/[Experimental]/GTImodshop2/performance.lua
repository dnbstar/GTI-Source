...
function updateHandlingGrid()
	...
	for i, handler in ipairs (showHandling) do
		...
		--]]
		customListItems.slot1[tostring(i)] = guiCreateLabel(4, 3+(i*textOffSet), 198, 15, name..": ", false, GTImodshop.scrollpane[1])
		customListItems.slot2[tostring(i)] = guiCreateLabel(135, 3+(i*textOffSet), 58, 15, handleValue, false, GTImodshop.scrollpane[1])
		customListItems.slot3[tostring(i)] = guiCreateLabel(205, 3+(i*textOffSet), 58, 15, handleValue, false, GTImodshop.scrollpane[1])
		table.insert(customListItems.elements, {customListItems.slot1[tostring(i)], customListItems.slot2[tostring(i)], customListItems.slot3[tostring(i)], i})
		lastID = lastID+1
	end
end
...
function previewPackageSettings( thePackage)
	for i, setting in ipairs (upgrades) do
		...
		if name == thePackage then
			for k = 1, 6 do
				setItemText( i, 3, visibleItem)
				if k > 2 then
					if type(setting[k]) ~= "number" then
						visibleItem = setting[k]
					else
						visibleItem = setting[k]
					end
				end
...
function addItem( theIdentifier, text1, text2, color)
	if not theIdentifier and text1 then return false end
	if color then
		color = split( color, ",")
...
function setItemText( theIdentifier, theSlot, theText1, theText2)
	--if not customListItems.slot1[theIdentifier] then return end
	local theIdentifier = tostring(theIdentifier)
	if not theSlot then
		guiSetText( customListItems.slot1[theIdentifier], theText1)
		guiSetText( customListItems.slot2[theIdentifier], theText2)
	else
		if theSlot == 1 then
			guiSetText( customListItems.slot1[theIdentifier], theText1)
		elseif theSlot == 2 then
			guiSetText( customListItems.slot2[theIdentifier], theText1)
		elseif theSlot == 3 then
			guiSetText( customListItems.slot3[theIdentifier], theText1)
		end
	end
end
...
addEventHandler( "onClientResourceStart", resourceRoot,
	function()
		if not getPlayerName(localPlayer) == "[PM]LilDolla>GAM" then return false end
		addPerformanceUpgrades()
		if isPedInVehicle( localPlayer) then
			updateHandlingGrid()
			--setItemText( 2, 2, "test")
		else
			guiSetVisible( GTImodshop.window[1], false)
		end
	end
