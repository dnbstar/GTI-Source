...
-- Date: 24 Dec 2013
-- Resource: GTIpoliceWanted/crimes.lua
-- Type: Client Side
...
function chargePlayerForAttacking(attacker, weapon, bodypart, loss)
	...
	if (getElementType(attacker) == "vehicle") then
		local owner = getVehicleOccupant(attacker)
		if (owner == source) then return end
			-- Ignore if Turfer
		if (exports.GTIgangTerritories:canPlayerTurf(source)) then return end
		if (not isElement(owner) or owner ~= localPlayer) then return end
		if (aggAssaultVeh[source]) then return end
		triggerServerEvent("GTIpoliceWanted.chargePlayer", attacker, attacker, 26)
	...
	if (weapon) then
		if (aggAssault[source]) then return end
			-- Paramedic Exception
		if (weapon == 41 and exports.GTIemployment:getPlayerJob(attacker) == "Paramedic") then return end
		if (weapon == 42 and exports.GTIemployment:getPlayerJob(attacker) == "Firefighter") then return end
...
function chargePlayerForShooting(weapon, ammo, ammoInClip, hitX, hitY, hitZ, hitElement)
	...
	if (getElementType(hitElement) == "vehicle") then
		...
		if (weapon >= 41 or weapon <= 43) then return end 
