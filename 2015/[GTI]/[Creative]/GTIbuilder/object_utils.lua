...
addEventHandler( "onClientRender", root,
	function()
		for i, object in pairs (objs) do
			local object = object[1]

			local x, y, z, pX, pY, pZ = getObjectLineSight( object)

			dxDrawLine3D( x, y, z+hitZ, pX, pY, pZ+hitZ, tocolor( 255, 255, 255, 200), 2)
			dxDrawLine3D( pX-hitS, pY, pZ+hitZ, pX+hitS, pY, pZ+hitZ, tocolor( 255, 0, 0, 200), 2) -- X
			dxDrawLine3D( pX, pY-hitS, pZ+hitZ, pX, pY+hitS, pZ+hitZ, tocolor( 255, 0, 0, 200), 2) -- Y

			if not isObjectOnGround( object) then
				--setObjectOnGround( object)
			end
		end
	end
)
...
addEventHandler("onClientRender", root,
    function()
		...
		if isElement( object) and setting then
			setElementPosition( object, wX, wY, wZ+2.5)
		end
		--]]
    end
)
...
addEventHandler( "GTIbuilder.data", root, datas)
...
addEventHandler( "onClientClick", root,
	function( button, state, aX, aY, wX, wY, wZ, entity)
		...
		if not showing then return end
		if button == "left" then
			local posString = tostring( wX..","..wY..","..wZ)
			triggerServerEvent( "GTIbuilder.data", localPlayer, "placeObject", "o", posString)
		elseif button == "right" then
			if setting then
				return outputChatBox( "You can't delete an object while placing another object.")
			end
			object = entity
			triggerServerEvent( "GTIbuilder.data", localPlayer, "deleteObject")
		end
	end
)
--]]

--[[ Texture Application Cursor
addEventHandler( "onClientClick", root,
	function( button, state, aX, aY, wX, wY, wZ, entity)
		if state ~= "down" then return end
		if not showing then return end
		if entity and getElementType( entity) == "object" then
			...
			if isObjectCustom( object) then
				...
				if button == "left" then
					...
					--triggerServerEvent( "GTIbuilder.shader", localPlayer, "apply", "digi2")
					triggerServerEvent( "GTIbuilder.data", localPlayer, "apply", i, "digi2")
				elseif button == "right" then
					--removeCustomTexture( object)
					triggerServerEvent( "GTIbuilder.data", localPlayer, "remove", i, "digi2")
				end
			end
		end
	end
