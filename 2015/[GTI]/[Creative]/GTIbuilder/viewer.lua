...
function randomizeObject()
	...
	if isElement( prevObject) then
		local object = math.random( #allIDs)
		local newObject = allIDs[object]
		setElementModel( prevObject, newObject)
		--
		local texture = math.random( #alltextures)
		local newTexture = alltextures[texture][1]
		if newTexture == "none" then
			local newObject = allcustom[object][1]
			removeCustomTexture( prevObject)
			applyCustomTexture( prevObject, "default")
		else
			applyCustomTexture( prevObject, newTexture)
		end
	end
end
setTimer( randomizeObject, 2500, 0)
--]]
