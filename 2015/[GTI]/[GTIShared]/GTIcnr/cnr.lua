...
events = {
    --{ Name, Abbreviation}
    [1] = { "Caligula's Casino Robbery", "GTIccr"},
    [2] = { "LS Police Raid", "GTIpdr"}
...
function playerKilled(ammo, attacker)
    if isElement( attacker) then
        if isPlayerInCnREvent( source) and isPlayerInCnREvent( attacker) then
            local aacc = getPlayerAccount( attacker)
            local sacc = getPlayerAccount( source)
            ...
            if not exports.GTIstats:getStatData( getPlayerAccount( attacker), "cnrKills") then
                exports.GTIstats:setStatData( aacc, "cnrKills", 1)
            else
                exports.GTIstats:modifyStatData( aacc, "cnrKills", 1)
            end
            if not exports.GTIstats:getStatData( getPlayerAccount( source), "cnrDeaths") then
                exports.GTIstats:setStatData( sacc, "cnrDeaths", 1)
            else
                exports.GTIstats:modifyStatData( sacc, "cnrDeaths", 1)
            end
                ...
                if tcL then
                    ...
                    if eventRunning then
                        ...
                        if getPlayerEventSide( hitElement) and getPlayerEventSide( hitElement) == "crim" then
                            ...
                            exports.GTIcriminals:givePlayerTaskMoney(hitElement, "CnR Robbery", extraM)
