local Chats = { "Support", "Main", "Team", "Local", "Group", "AR", "ES", "PT", "NL", "TN", "TR", "Joinquit", }
local chatSystem = {}
local eTable = {}
...
guiSetPosition(panel,x,y,false)
    ...
    for k,v in ipairs ( Chats ) do
        ...
        chatSystem[chat][3] = guiGridListAddColumn(chatSystem[chat][1], "Time", 0.15)
        chatSystem[chat][4] = guiGridListAddColumn(chatSystem[chat][1], "Message", 0.8)
        addEventHandler( "onClientGUIDoubleClick", chatSystem[chat][1], copyRow, false )
        if (chat == "Joinquit")  then return end
        ...
        guiGridListSetSelectionMode( chatSystem[chat][1], 0 )
    ...
    setTimer ( function ( )
        ...
        if ( file ) then
            for i, v in ipairs(xmlNodeGetChildren(file)) do
                ...
                if ( tostring( enabled ):lower () ~= "false" ) then
