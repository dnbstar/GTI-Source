...
function onCSZEnter(player, zone)
	if (player ~= localPlayer) then return end
	if (not source) then source = zone end
...
function isPlayerInSafeZone( plr)
	if pisz[plr] then
		return true
	else
		return false
...
function onCSZExit( player)
	...
	--if not isPlayerInVehicle( player) then
		...
		--addEventHandler( "onClientPlayerDamage", player, eventCancel, false)
...
addEventHandler( "onClientSafeZoneEnter", root,
	function( source, name, status1, status2, interior)
		...
		if status1 == true then
			--If the player is a police officer, allow nightstick--
			if (exports.GTIutil:isPlayerInTeam(source, "Law Enforcement", "National Guard")) then
				setPedWeaponSlot(source,1)
				toggleControl("fire",true)
			else
			...
			if interior == false then
				if not isTimer( enter1Timer) then
					--exports.GTIhud:dm( "You entered the "..name.." Safezone.", 50, 100, 100) (Do not enable or make a settings system at add it as a setting)
			...
			if interior == false then
				if not isTimer( enter1Timer) then
					...
					--exports.GTIhud:dm( "You are now allocated in the "..name..".", 50, 100, 100) (Do not enable or make a settings system at add it as a setting)
					enter1Timer = setTimer( function() end, 10000, 1)
				end
			else
				--exports.GTIhud:dm( "You entered "..name..".", 100, 100, 100)
		...
		if status1 == true then
			...
			if interior == false then
				if not isTimer( leave1Timer) then
					--exports.GTIhud:dm( "You left the "..name.." Safezone.", 100, 100, 50) (Do not enable or make a settings system at add it as a setting)
